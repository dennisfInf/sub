...
namespace NetworkingClient
{
    ...
    public:
        ...
        explicit session(
            ...
            ssl::context &ctx, http::request<http::vector_body<uint8_t>> &_req)
            : resolver_(ex), stream_(ex, ctx), req_(_req)
        {
        }
    ...
    /* {
         ...
         char buffer[32];                        // Buffer to hold the string representation
         std::memset(buffer, 0, sizeof(buffer)); // Clear the buffer

         // Convert the integer value to a string
         std::sprintf(buffer, "%hu", conf.port);
         ...
         std::make_shared<session>(
             net::make_strand(ioc),
             ctx,)
             ->run(conf.address.to_string().c_str(), charPtr, "/test");
